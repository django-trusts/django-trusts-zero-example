...
# Adopt handle.register pair (issue #131 E1)

This record covers the example after Core **#131 C1 / PR #158**
(`e9fd4cd4f77624f3d5351b505808c1d6fa8bcbc4`) and Zero **#131 Z1 /
PR #31** (`fb32d70e82f6a63d03287eb959732db52bd266c8`). Core and Zero
`migrates.md` on those revisions are the library contract. This file
records **example** old/new behavior only.

Repository `master` and tag `dev_split_core_attempt_1` stay on the
pre-Zero baseline `2ee36f933618028f879ca9f5bce16495edeeed95`. E1 does
not rewrite or fast-forward either. The executable example remains
`dev`.

Authorization data and seeded Alice/Bob/Carol/Dave identities stay the
same. The host still does not invent a local `handle.register` /
`Ref` / `Along` relation. ZeroConfig donates Trust-as-content through
the merged handle API. Host `Content` subclasses still need Zero's
published terminal helper; without it Project `has_perm` / `.permitted()`
fail closed.
...
| | |
| --- | --- |
| Previous | `django-trusts-zero` at `bceb0241b482fdc4f31dd72c7600c52eeb4e6cff` (PR #26 Z-convert) depending on `django-trusts` at `710b3ea26778ff069d1f5329adc9f2f481a1ea92` (PR #144 Core Stage A). Historical `master` still pins monolithic `django-trusts` at `8916a760fbe849170e88e3969723b317d0360cd1` only. |
| New | `django-trusts-zero` at `fb32d70e82f6a63d03287eb959732db52bd266c8` (PR #31 Z1) depending on `django-trusts` at `e9fd4cd4f77624f3d5351b505808c1d6fa8bcbc4` (PR #158 C1 `handle.register`). |
| Replacement | Same git URLs, new full SHAs in `requirements.txt` / `pyproject.toml`. No floating branch. |
| Affected | Docs, pin proofs, `manage.py check` against the exact pair. Seed, views, TrustGroup UI, and list SQL are unchanged. |
| Authorization | Unchanged seeded allow/deny. Trustee and TrustGroup intersection, list/object agreement, fail-closed anonymous/inactive, and fixed query bounds stay the same. |

## Old → new

```python
# Old (E-convert / #15 on dev — Zero helper against the registry)
from trusts.zero.registration import register_zero_content
registry = handle.registry
register_zero_content(registry, Project)

# Old (master 2ee36f93 — monolithic pin; host ready() is signals only)
INSTALLED_APPS = [..., "trusts", "projects.apps.ProjectsConfig"]
AUTHENTICATION_BACKENDS = ["trusts.backends.TrustModelBackend"]
# django-trusts @ …@8916a760fbe849170e88e3969723b317d0360cd1
from . import signals  # ProjectsConfig.ready() — no local donation

# New (E1 — exact C1 + Z1 pair; helper takes the backend handle)
INSTALLED_APPS = [..., "trusts.zero.apps.ZeroConfig", "projects.apps.ProjectsConfig"]
AUTHENTICATION_BACKENDS = ["trusts.zero.backends.TrustModelBackend"]
# django-trusts @ …@e9fd4cd4f77624f3d5351b505808c1d6fa8bcbc4
# django-trusts-zero @ …@fb32d70e82f6a63d03287eb959732db52bd266c8
from trusts.zero.registration import register_zero_content
handle = zero_config().configured_backend(CANONICAL_BACKEND_PATH)
register_zero_content(handle, Project)
# Do not: from trusts.core import Ref
# Do not: handle.registry.register(...)
# Do not: handle.registry.register_strategy(...)
# Do not: Along(...)
# Do not: register_zero_content(registry, Project)
# Do not invent handle.register(CustomGrant, ...) in the example
```

Unchanged public call sites: `User.has_perm`, `Project.objects.permitted`,
`trusts.decorators.permission_required` + `K("pk")`, trustee /
TrustGroup writers, `Trust:own` builder donation by ZeroConfig.

## Changes (example)

### 15. Dependency pair is the reviewed Core C1 + Zero Z1 heads

| | |
| --- | --- |
| Previous | Zero PR #26 + core PR #144 on `dev`. Monolithic core-only pin on `master`. |
| New | Exact pair above. Packaged install must resolve those git commits (`direct_url.json`). |
| Replacement | Same install files; new SHAs. CI asserts both commit ids after install. |
| Affected | `requirements.txt`, `pyproject.toml`, CI install step, README, `docs/TRUSTS_FIT.md`. |
| Authorization | Unchanged once the pair is installed. A mixed or floating pin fails closed or installs the wrong public API. |

### 16. Host Project donation uses the backend handle

| | |
| --- | --- |
| Previous | `dev` E-convert called `register_zero_content(registry, Project)`. `master` ready() imported signals only because monolithic TrustsConfig registered every `Content` subclass. |
| New | `ProjectsConfig.ready()` still imports signals, then donates Project with `register_zero_content(handle, Project)`. Zero Z1 helpers require a `BackendHandle` (`TypeError` on a bare registry). ZeroConfig donates Trust-as-content only; a missing Project terminal fails closed (empty lists / deny). This is Zero's published content helper, not a local `handle.register` / `Ref` / `Along` / `.registry.register` relation. |
| Replacement | `zero_config().configured_backend(CANONICAL_BACKEND_PATH)` then `register_zero_content(handle, Project)`. |
| Affected | `projects/apps.py`. Views, seed, and grants are unchanged. |
| Authorization | Same grants as the E-convert pin. Project plans are TUP plus both TGP alternatives, same as Trust. |

## Fail-closed rollout

1. Example `dev` stays at `0cb7ea23708610e467915abd4c2f768b5aece839` until this PR. `master` stays at `2ee36f933618028f879ca9f5bce16495edeeed95`.
2. Pin Core `e9fd4cd4…` and Zero `fb32d70e…` together. Do not pin only one. Do not pin a floating branch.
3. Convert `register_zero_content` to the handle in the same PR as the pin bump.
4. `python manage.py check` and the full SQLite / MySQL matrix must be green before review.
5. G1, C1-fold, W1, C2, #146, #159/#160, and release/version work stay gated.

## Rollback

Revert this example PR (or restore the previous `dev` pin pair and
`register_zero_content(registry, Project)`). Do not rewrite `master` or
tag `dev_split_core_attempt_1`. No Trusts schema or seed rewrite is
involved, so rollback does not migrate data.

Advertised-callback removal stays on the historical route: leftover
`TRUSTS_ALLOW_LEGACY_PERMISSION_CALLBACKS=True` is `trusts.E007` and
does not restore runtime callbacks.

## Migration-bot checklist

- [ ] Confirm `requirements.txt` / `pyproject.toml` pin Core
      `e9fd4cd4f77624f3d5351b505808c1d6fa8bcbc4` and Zero
      `fb32d70e82f6a63d03287eb959732db52bd266c8` (full SHAs).
- [ ] Confirm CI install asserts both `direct_url.json` commit ids.
- [ ] Confirm `INSTALLED_APPS` has `trusts.zero.apps.ZeroConfig` and
      not bare `"trusts"`.
- [ ] Confirm `AUTHENTICATION_BACKENDS` has
      `trusts.zero.backends.TrustModelBackend` and not
      `trusts.backends.TrustModelBackend`.
- [ ] Search application modules (`projects/`, `example/`, excluding
      tests) for `from trusts.core import Ref`, `Ref(`,
      `.registry.register(`, `.registry.register_strategy(`, `Along(`,
      `register_zero_content(registry`. Those forms must be gone.
- [ ] Confirm `ProjectsConfig.ready()` donates Project via
      `register_zero_content(handle, Project)` and still imports
      signals. Do not add `handle.register(CustomGrant, …)`.
- [ ] Confirm concrete models / authorization helpers / policy stay on
      `trusts.zero.*`. Decorators stay on core `trusts.decorators`.
- [ ] Confirm `Trust:own` object and listing decisions still agree.
- [ ] Confirm Project has no `:own` (unknown code fails closed).
- [ ] Confirm trustee + TrustGroup intersection, list/object agreement,
      fail-closed anonymous/inactive, and filter-before-pagination.
- [ ] Confirm `python manage.py check` is clean of `trusts.E001` /
      `trusts.E007`.
- [ ] Keep `permission_required` + `K("pk")`. Do not adopt
      `require_authorized` here (#138).
...
- [ ] Do not rewrite or fast-forward `master` or
      `dev_split_core_attempt_1`.
- [ ] Do not start G1, C1-fold, W1, C2, #146, #159/#160, release/version
      work, or deployment (#7/#13) in this PR.

## Deployment / migration checklist (example)

Deployment remains separately owned by #7/#13. This issue creates no
infrastructure action. Do not deploy or push to Dokku from E1.

- [ ] Install Zero at `fb32d70e82f6a63d03287eb959732db52bd266c8` and
      core at `e9fd4cd4f77624f3d5351b505808c1d6fa8bcbc4`.
- [ ] Deploy later from `dev` if an operator chooses; do not advance
      the preserved `master` branch.
- [ ] `python manage.py check` (must be clean of `trusts.E001` /
      `trusts.E007`).
...
- G1 GH permissions
- C1-fold `handle.register_strategy` / `OrderedFold`
- W1 Windows ACL
- C2 `handle.registry` → `_registry`
- Runtime callbacks, portable JSON, registration identity (#145)
- `require_authorized` / decorator inference (#138)
- Deployment / infrastructure (#7/#13)
- #146, #159/#160, release/version work
- #137, unrelated cleanup
